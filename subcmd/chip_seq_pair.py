...
def arg_chip_seq_pair(parser):
	cmd=parser.add_parser('chip_seq_pair',help='Paired-end ChIP-seq pipeline',formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def run_chip_seq_pair(args,rootLogger):
	# rootLogger.info("this is chip_seq_pair")
	# rootLogger.info(str(args.subcmd).lower())
	if str(args.subcmd).lower() != "chip_seq_pair":
		return 0
	if args.guess_input:
	...
	if not (os.path.isfile(args.input) or not os.path.isfile(args.design_matrix)):
		...
		os.system("HemTools chip_seq_pair -h")
		# rootLogger.close()
		os.system("rm "+args.jid+".log")
		sys.exit(1)
	...
	chip_seq_pair = NGS_pipeline(args,rootLogger)
	
	
	chip_seq_pair.parse_paired_fastq()
	chip_seq_pair.parse_treatment_control()
	chip_seq_pair.dry_run()

	chip_seq_pair.run_fastqc()
	chip_seq_pair.run_BWA()
	chip_seq_pair.run_samtools_markdup_rmdup()
	chip_seq_pair.run_spp()

	
	# run_bamCoverage
	chip_seq_pair.run_bamCoverage('${COL3}.markdup.bam','${COL3}.all.bw','all')
	chip_seq_pair.run_bamCoverage('${COL3}.rmdup.bam','${COL3}.rmdup.bw','rmdup')
	chip_seq_pair.run_bamCoverage('${COL3}.rmdup.uq.bam','${COL3}.rmdup.uq.bw','rmdup.uq')

	# lib complexity 
	chip_seq_pair.run_lib_complexity()

	chip_seq_pair.run_macs2_narrowPeak()

	chip_seq_pair.run_STJtracks()

	chip_seq_pair.run_output_organization()
	if args.debug:

...
################################################################################################
### s3norm
def s3norm(sig1_wg_raw, sig2_wg_raw, sig2_output_name, NTmethod, B_init, fdr_thresh, rank_lim, upperlim, lowerlim, p_method):
...
def main(argv):
	...
	opts, args = getopt.getopt(argv,"hr:t:o:m:i:f:l:a:b:p:")
	for opt,arg in opts:
		if opt=="-h":
			print('time python ../src/s3norm.py -r reference.bedgraph -t target.bedgraph -o target -m (Method for matching peaks and background: non0mean, non0median, mean, median) -i initial_B -f FDR_thresh -l rank_lim_p -a upperlimit -b lowerlimit -p (p-value_method: neglog10p, z)')
			return()	
		elif opt=="-r":
	...
	######### run s3norm
	print('start S3norm.......')
	s3norm(sig1_wg_raw, sig2_wg_raw, sig2_output_name, NTmethod, B_init, fdr_thresh, rank_lim, upperlim, lowerlim, p_method)

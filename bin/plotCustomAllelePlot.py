...
# plotCustomAllelePlot.py -f pAC128_uKJ003_rep2_S80/CRISPResso_on_pAC128_uKJ003_rep2_S80_R1_001_pAC128_uKJ003_rep2_S80_R2_001 -a pAC128_uKJ003_rep2_S80.raw.count.tsv -o test -pam NGGNN
def main():
    ...
    parser.add_argument('-a',"--allele_table",type=str,help="CRISPResso output folder containing finished analysis",required=True)
    parser.add_argument('-pam',"--PAM_seq",type=str,help="input PAM sequence (5bp) to subset allele table, NGGNN",default="")
    ...
    parser.add_argument("--plot_left",help="Number of bases to plot to the left of the cut site",type=int,default=50)
    parser.add_argument("--plot_right",help="Number of bases to plot to the right of the cut site",type=int,default=50)
    ...
    if args.PAM_seq == "":
        plot_alleles_tables_from_folder(args.allele_table,args.CRISPResso2_folder,args.output_root,MIN_FREQUENCY=args.min_freq,MAX_N_ROWS=args.max_rows,SAVE_ALSO_PNG=args.save_png,plot_cut_point=args.plot_cut_point,plot_left=args.plot_left,plot_right=args.plot_right,args=args)
    else:
        df = pd.read_csv(args.allele_table,sep="\t")
        df['myPAM'] = df.actual_PAM.apply(lambda x:rename_PAM(x,args.PAM_seq))
        df2 = df[df.myPAM==args.PAM_seq]
        total = df2['#Reads'].sum()
        df2['%Reads'] = [x*100/total for x in df2['#Reads']]
        out = f"{args.output_root}.tsv"
        df2.to_csv(out,index=False,sep="\t")
        plot_alleles_tables_from_folder(out,args.CRISPResso2_folder,args.output_root,MIN_FREQUENCY=args.min_freq,MAX_N_ROWS=args.max_rows,SAVE_ALSO_PNG=args.save_png,plot_cut_point=args.plot_cut_point,plot_left=args.plot_left,plot_right=args.plot_right,args=args)
def rename_PAM(x,PAM_string="NXXNN"):
    out = []
...
def plot_alleles_tables_from_folder(allele_table,crispresso_output_folder,fig_filename_root,plot_left=20,plot_right=20,MIN_FREQUENCY=None,MAX_N_ROWS=None,SAVE_ALSO_PNG=False,custom_colors=None,plot_cut_point=True,sgRNA_intervals=None,sgRNA_names=None,sgRNA_mismatches=None,args=None):
    """
    ...
    df_alleles = pd.read_csv(allele_table,sep="\t")

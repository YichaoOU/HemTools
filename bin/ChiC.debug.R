...
Geometric_mean=function(table=table,cis_window=1500000) {
	...
	start=seq(0,cis_window,by=30000)
	end=seq(30000,cis_window+30000,by=30000)
	# print (start)
	# print (end)
...
FindPeaks <- function(table=table,w=50,span=0.05,d=0,cis_window=1500000) {
  y=table[,"N"]
	...
	for (j in 1:dim(table)[1]){
		...
		# print (dist)
		# print (tail(distance_d))
		value=distance_d[dist >distance_d$start & dist <= distance_d$end,]
		# value=value[!is.na(value$predicted),]
		# print (value)
		peak$predicted = value$predicted

		# final[[j]] = peak
		final <- append(final, list(peak))
		
	}
	# print (final)
	final=Reduce(rbind,final)
	# print (final)
	return (final)
...
cat(file=hits.fn,"ID_Bait","\t","chr_Bait","\t","start_Bait","\t","end_Bait","\t","Bait_name","\t","ID_OE","\t","chr_OE","\t",  "start_OE","\t","end_OE","\t","OE_name","\t","N","\t","predicted_value","\n")

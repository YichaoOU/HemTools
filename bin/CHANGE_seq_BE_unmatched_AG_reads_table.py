...
file=f"{label}_identified_unmatched.txt"
...
def get_read_list2(r,start,end): # for unmatched data
	if r.strand1=="+":
...
def get_distance_to_overlap_end(r):
	...
	return out   
def get_AG_vector(read,OT_name,chr,start,end,seq,strand,RC):
	...
	q = read[read.chr==chr]
	q = q[q.is_outie==True]
	q["is_site"] = q.apply(lambda r:get_read_list2(r,start,end),axis=1)
	q["second_overlap_start"] = q.apply( get_overlap_second_start_position,axis=1)
	q = q[q.is_site==True]
...
for i,r in df.iterrows():
    ...
    strand = "+"
    RC = r[4]
    tmp,tmp_q = get_AG_vector(read,r[3],chr,start,end,seq,strand,RC)
    # out2.append(tmp_q)
    tmp.columns = [r[3]]
    # print (r[c],"finished")
    out.append(tmp)
...
df.to_csv(f"{label}.unmatched.A_to_G_frequency.csv")
AG = pd.read_csv(f"{label}.unmatched.A_to_G_frequency.csv",index_col=0)
...
df2.to_csv(f"{label}_identified_unmatched.add_AG_frequency.csv")

df = pd.concat(out2)
df.to_csv(f"{label}.unmatched.A_to_G_overlap_distance.csv")
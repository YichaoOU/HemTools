...
print (df.head())
df[0] = df["chr1"]+"_"+df["s1"].astype(str)+"_"+df["e1"].astype(str)
df[1] = df["chr2"]+"_"+df["s2"].astype(str)+"_"+df["e2"].astype(str)
print (df.head())
label_user = sys.argv[2]
G=nx.from_pandas_edgelist(df, 0, 1)
print(nx.info(G))
# print (len(G.nodes()))
# nx.write_gml(G, "filtered_network_1019.gml")
node_features = pd.DataFrame(list(G.nodes))
node_features[1] = 1

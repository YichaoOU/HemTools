...
def demulti(file,outFile,DNA_BC,RNA_BC,num_mismatch):
	...
	for read in fq(file):
		...
		if RNA_dist<=num_mismatch: # RNA_read
			...
			DNA_dist = distance(R2[:8], DNA_BC)
			if DNA_dist<=num_mismatch:
				outlines_DNA.append(name+"\n")
				outlines_DNA.append(read[1])
				outlines_DNA.append(read[2])
				outlines_DNA.append(read[3])
	output_to_fastq_gz(outFile.replace("type","RNA."+RNA_BC),outlines_RNA)
	output_to_fastq_gz(outFile.replace("type","DNA."+DNA_BC),outlines_DNA)

DNA_BC=sys.argv[1]
RNA_BC=sys.argv[2]
num_mismatch=int(sys.argv[3])


R1="Undetermined_S0_R1_001.fastq.gz"
R2="Undetermined_S0_R2_001.fastq.gz"

demulti(R1,"type.R1.fastq.gz",DNA_BC,RNA_BC,num_mismatch)
demulti(R2,"type.R2.fastq.gz",DNA_BC,RNA_BC,num_mismatch)
...
sleuth_table <- sleuth_table[order(sleuth_table[,"pval"]),c("target_id", "pval", "qval", "ens_gene", "ext_gene")]
write.csv(sleuth_table, file="{{output_name}}.transcript.sleuth.csv", row.names=FALSE)

# df = sleuth_to_matrix(so,"obs_norm","tpm")
df = sleuth_to_matrix(so,"obs_raw","tpm")
write.csv(df, file="{{output_name}}.transcript.normalized.tpm.csv", row.names=T)
# df = sleuth_to_matrix(so,"obs_norm","est_counts")
df = sleuth_to_matrix(so,"obs_raw","est_counts")
write.csv(df, file="{{output_name}}.transcript.normalized.est_counts.csv", row.names=T)


so <- sleuth_prep(sample_info, ~condition, target_mapping = t2g, aggregation_column = 'ens_gene',gene_mode = TRUE)
so <- sleuth_fit(so, ~condition, 'full')
so <- sleuth_fit(so, ~1, 'reduced')
so <- sleuth_lrt(so, 'reduced', 'full')
sleuth_table <- sleuth_results(so, 'reduced:full', 'lrt', show_all = TRUE)
sleuth_table <- sleuth_table[order(sleuth_table[,"pval"]),c("target_id", "pval", "qval", "ext_gene")]
write.csv(sleuth_table, file="{{output_name}}.gene.sleuth.csv", row.names=FALSE)

# df = sleuth_to_matrix(so,"obs_norm","tpm")
df = sleuth_to_matrix(so,"obs_raw","tpm")
write.csv(df, file="{{output_name}}.gene.normalized.tpm.csv", row.names=T)
# df = sleuth_to_matrix(so,"obs_norm","est_counts")
df = sleuth_to_matrix(so,"obs_raw","est_counts")
write.csv(df, file="{{output_name}}.gene.normalized.est_counts.csv", row.names=T)
...
def prepare_sample_info(conditions):
...
def main(R_command):
	...
	if args.sample_info == "None":
		args.sample_info,args.treatment_list,args.control_list = prepare_sample_info([args.treatment,args.control])

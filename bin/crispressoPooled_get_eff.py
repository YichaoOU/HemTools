...
for s in sample_id_list:
	...
	for name,_,gRNA in df.values:
		...
		file = "{0}_results/CRISPRessoPooled_on_{0}/CRISPResso_on_{1}/Nucleotide_percentage_table.txt".format(s,correct_name(name))
		myLength = len(gRNA)
		# print (name,_,gRNA)
	...
	file = "{0}_results/CRISPRessoPooled_on_{0}/SAMPLES_QUANTIFICATION_SUMMARY.txt".format(s)

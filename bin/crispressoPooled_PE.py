...
def my_args():
	...
	mainParser.add_argument('-a','--amplicon_bed',  help="amplicon_bed required, name column need to match gRNA bed",required=True)
	mainParser.add_argument('-gRNA','--gRNA_bed',  help="gRNA_bed required, name column need to match amp bed",required=True)
	mainParser.add_argument('-f','--fastq_tsv',  help="gRNA_bed required",required=True)
	mainParser.add_argument('--RTT',  help="reverse transcription template. Note, this sequence is used to check substitution rate caused by prime editing, so for this parameter, you should use the reverse complement sequence of the actual RTT sequence in pegRNA",required=True)

	# mainParser.add_argument('--cas9',  help="not running in base editor mode",action='store_true')
	mainParser.add_argument('--remove_PAM',  help="gRNA bed coord include PAM, but crispresso need to remove PAM",action='store_true')
...
def prepare_input(amp_bed,gRNA_bed,fasta,jid,remove_PAM=False):
	df = pd.read_csv(amp_bed,sep="\t",header=None,comment="#")
	df2 = pd.read_csv(gRNA_bed,sep="\t",header=None,comment="#")
	# make input file for crispressoPooled, 3 column tsv, amp Name, amp Seq, gRNA Seq
	# get gRNA sequence
	out_fa = "gRNA.fa"
	...
	df3.index = [x.split("::")[0] for x in df3.index] # store gRNA seq
	if remove_PAM:
		df3[1] = [x[:-3] for x in df3[1]]
	df3[2] = [len(x) for x in df3[1]]

	print ("gRNA bed")
	print (df3.head(n=2))
	print (df3[2].value_counts())
...
def main():
	...
	df=df[[0,1]]
	df[2] = "%s_info.tsv"%(args.jid)
	df.to_csv("%s.input"%(args.jid),sep="\t",header=False,index=False)
	prepare_input(args.amplicon_bed,args.gRNA_bed,args.genome_fasta,args.jid,args.remove_PAM)	

	args.input_list = "%s.input"%(args.jid)
	os.system("mkdir %s"%(args.jid))
	os.system("mkdir %s/log_files"%(args.jid))

	submit_pipeline_jobs(myPipelines["crispressoPooled_PE"],args)

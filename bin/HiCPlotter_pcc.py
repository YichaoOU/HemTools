...
def HiCplotter(files=[],names=[],resolution=100000,chromosome='',output='',histograms=[],histLabels=[],fillHist=[],histMax=[],verbose=False,fileHeader=0,fileFooter=0,matrixMax=0,histColors=[],barPlots=[],barLabels=[],plotGenes='',superImpose=False,anchor=0,anchorMax=0,\
	...
	cmaps = ['Greys','Reds','YlOrBr','bwr','bwr']
	h_start = []
	h_end = []
	...
	for exp in range(0,rlength):
		...
		if exp==0: 
			if not randomBins and not wholeGenome: ax1.set_ylabel('Pearson Matrix - %s Mb (resolution: %sKb)' % (chromosome , resolution/1000))
		...
		if numOfrows <= rowcounter and not randomBins and not wholeGenome: 
			...
			cbar = plt.colorbar(img, cax=cax, ticks=MultipleLocator(0.5), format="%.1f",orientation='horizontal',extendfrac='auto',spacing='uniform')
		...
		if len(histograms)>0:
			if not superImpose: # improve this part
				for x in range(0,len(histograms[0].split(','))):
					...
					else: ax3.plot(x_comps,y_comps,color='grey',alpha=0.1)
					...
					ax3.locator_params(axis='y',tight=False, nbins=6)
					ax3.set_xlim(int(start or 1) - 0.5,int(start or 1) + length - 0.5)
					print (ax3.get_yticks().tolist())
					print (ax3.get_xticks().tolist())
						...
						if len(histColors)>0:
							if histColors[exp].split(',')[x] != '':
								...
								if ymin < 0: 
									...
									with np.errstate(all='ignore'):ax3.fill_between(x_comps, comps2, 0, where=comps2<0, color='blue', interpolate=True)
							else:

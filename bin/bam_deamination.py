...
def my_args():
	...
	mainParser.add_argument('-o',"--output",  help="output prefix",default="bam_deamination_pos_"+username+"_"+str(datetime.date.today())+".bed")
...
def main():
	...
	for k in df:
		...
		if len(out1) == 0 and len(out2) == 0:
			# no deamination skip
			continue
		deamination_pos.append(out1 + out2)

		flag,v = is_outie(df[k][0],df[k][1])
		outtie.append(flag)
		overlap.append(v)
		qnames.append(k)
		c,s,e = get_mapping_location(df[k][0],df[k][1])
		chr.append(c)
		start.append(s)
		end.append(e)
	...
	df['#end'] = end
	df['#Read_name'] = qnames
	df['#deamination_pos'] = deamination_pos
	df['#outward_flag'] = outtie
	df['#overlap'] = overlap
	df['#outward_flag'] = df['#outward_flag'].astype(int)
	df = df[df['#outward_flag']==1]
	df = df.explode("#deamination_pos")
	# df['R1'] = read1
	# df['R2'] = read2

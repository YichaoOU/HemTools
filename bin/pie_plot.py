...
def pie_chart(char_list,value_list,output):
	...
	# adjust_text(t1)
	if df1.shape[0] > 0:
		plt.title("These categories are not found: %s"%(", ".join(df1[0].tolist())))
...
def main():
	...
	if args.just_plot:
		...
		pie_chart(char_list,value_list,args.output)
	...
	my_cat = df[args.use_col].value_counts(normalize=True).to_dict()

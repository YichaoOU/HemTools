...
for s,d in dff.groupby(["cluster","sample_name"]):
	print (s)
	d.index = d.gene.tolist()
	out = f"{s[0]}.{s[1]}"
	run_enrichR(d[d.p_val_adj<=0.01].gene.tolist(),genesets,out_file)
	rnk = d[['avg_log2FC']].reset_index()
	print (rnk)
	for g in genesets:
		try:
	...
	os.system(f"mv GSEA_Prerank/ {out}")
	os.system(f"mv Enrichr/ {out}")
	os.system(f"mv GSEA_plots_FDR_0.01/ {out}")
